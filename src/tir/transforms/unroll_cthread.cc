// Unrolls threadloop as in Halide pipeline.
...
struct UnrollCthreadConfigNode : public tvm::AttrsNode<UnrollCthreadConfigNode> {
  int auto_max_step;
  int auto_max_depth;
  int auto_max_extent;
  int explicit_unroll;

  TVM_DECLARE_ATTRS(UnrollCthreadConfigNode, "tir.transform.UnrollCthreadConfig") {
...
class UnrollCthreadConfig : public Attrs {
 public:
  TVM_DEFINE_NOTNULLABLE_OBJECT_REF_METHODS(UnrollCthreadConfig, Attrs, UnrollCthreadConfigNode);
};

TVM_REGISTER_NODE_TYPE(UnrollCthreadConfigNode);
TVM_REGISTER_PASS_CONFIG_OPTION("tir.UnrollCthread", UnrollCthreadConfig);

class CthreadUnroller : public StmtExprMutator {
 public:
  explicit CthreadUnroller(int auto_max_step, int auto_max_depth, int auto_max_extent,
  ...
  Stmt VisitStmt_(const ForNode* op) {
    ...
    if ((auto_unroll && explicit_unroll_) ||
      ...
      if (auto_unroll) {
        if (op->kind != ForKind::kThreadBinding) {
          return For(op->loop_var, op->min, op->extent, ForKind::kThreadBinding, op->body,
  ...
  Stmt Unroll(const ForNode* op) {
    ...
    for (int i = 0; i < value; ++i) {
      PrimExpr current = analyzer_.Simplify(op->min + make_const(op->loop_var.dtype(), i));
      auto v = IntImm(op->loop_var.dtype(), current.as<IntImmNode>()->value);
      vmap.Set(op->loop_var, v);
      // Substitute: sugar to traverse stmts in body, replacing all v.key with v.value
      Stmt step = Substitute(body, vmap);
      unrolled.push_back(step);
    }
    // Flattern generated SeqStmt with unrolled Array
    Stmt ret = SeqStmt::Flatten(unrolled);
    VLOG(2) << "flattened: " << PrettyPrint(ret);
    return ret;
  }
...
Stmt UnrollCthread(Stmt stmt, UnrollCthreadConfig cfg) {
  Stmt ret = CthreadUnroller(cfg->auto_max_step, cfg->auto_max_depth, cfg->auto_max_extent,
...
Pass UnrollCthread() {
  auto pass_func = [=](PrimFunc f, IRModule m, PassContext ctx) {
    ...
    auto cfg = ctx->GetConfig<UnrollCthreadConfig>("tir.UnrollCthread");
    if (!cfg.defined()) {
      cfg = AttrsWithDefaultValues<UnrollCthreadConfig>();
    }
    n->body = UnrollCthread(std::move(f->body), cfg.value());
    return f;
  };
  return CreatePrimFuncPass(pass_func, 0, "tir.UnrollCthread", {});
}

TVM_REGISTER_GLOBAL("tir.transform.UnrollCthread").set_body_typed(UnrollCthread);
