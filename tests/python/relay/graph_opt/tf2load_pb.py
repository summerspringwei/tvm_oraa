...
sys.path.extend([
    '', '/home/xiachunwei/Software/0.7-tvm/tvm/python',
    '/home/xiachunwei/Software/tensor-compiler/src/xla',
    ...
    '/home/xiachunwei/Software/pytf2.10/lib/python3.7/site-packages',
...
def load_and_run(model_file, inputs, outputs, dtype="float32"):
    ...
    with tf.device("/device:gpu:0"):
        ...
        input_str_list = [
            name + ":0" for (name, shape) in input_name_shape_list
        ]
        ...
        for name, shape in input_name_shape_list:
            tensor = tf.constant(np.random.random((shape)).astype(dtype),
                                 dtype=dtype,
                                 shape=shape,
                                 name=name)
